...
class AccountSettingsViewModel(val account: Account) : GenericSettingsViewModel() {
    ...
    val userNameListener = object : SettingListenerStub() {
        override fun onTextValueChanged(newValue: String) {
            ...
            if (identity != null) {
                ...
                val newIdentityAddress = Factory.instance().createAddress("sip:$newValue")
                if (newIdentityAddress != null) {
                    params.identityAddress = newIdentityAddress
                    account.params = params
                } else {
                    Log.e(
                        "[Account Settings] Failed to create identity address sip:$newValue"
                    )
                }
            }
        }
    ...
    private fun update() {
        ...
        if (identityAddress != null) {
            ...
            identity.value = "${identityAddress.displayName ?: ""}\n${identityAddress.username ?: ""}"
        }
